...
## 🏗 2. Managed PostgreSQL Setup (Azure Portal / CLI)
...
1.  **Create Server**:
    - If you ran the automated `setup_azure_infra.sh` script, this was provisioned automatically in Resource Group `rg-test-3` with a name like `psql-cardcanvas-<random-id>`.
    - If manually creating:
      - **Resource Group**: `rg-test-3` (or your custom name).
      - **Server name**: `psql-cardcanvas-<unique-id>`.
...
2.  **Networking & Firewall Configuration**:
    - Under **Networking**, choose **Public access (allowed IP addresses)**.
    ...
    - Add your **Local Development IP address** to the whitelist so you can seed the database schemas. (Note: The `setup_azure_infra.sh` script automatically whitelists `0.0.0.0/0` to allow all IP addresses for temporary testing).
3.  **Enable Extensions**:
...
4.  **Database & Schema Seeding**:
    - From your local terminal, connect to PostgreSQL (replace `<DATABASE_FQDN>` with your DB Host Endpoint):
      ```bash
    ...
    - Create the production database:
      ...
      \q
      ```
    - **Note on Migrations**: The Rust backend is configured to automatically run SQLx migrations on startup. You do not need to manually apply the migration SQL files. Simply creating the `cardcanvas` database is sufficient!
    - *(Optional Manual Verification)* If you wish to apply them manually anyway:
      ```bash
...
2.  **SSH**: Connect to the VM (using the key generated by `setup_azure_infra.sh`):
    ```bash
    ssh -i ~/.ssh/id_rsa_cardcanvas azureuser@<BACKEND_PUBLIC_IP>
    ```
...
1.  **Transfer Source Code**: From your local workspace, package and transfer the backend (using `COPYFILE_DISABLE=1` and excluding `._*` metadata to avoid compile-time SQLx macro issues):
    ```bash
    COPYFILE_DISABLE=1 tar --exclude="target" --exclude=".git" --exclude="uploads" --exclude="._*" -czf backend-src.tar.gz cardcanvas-backend
    scp -i ~/.ssh/id_rsa_cardcanvas backend-src.tar.gz azureuser@<BACKEND_PUBLIC_IP>:/tmp/
    ```
...
3.  **Environment Configuration**:
    ...
    PORT=8080
    FRONTEND_URL=http://<FRONTEND_PUBLIC_IP> # Or your custom domain e.g., https://cardcanvas.com
    ```
...
2.  **SSH**: Connect to the VM (using the key generated by `setup_azure_infra.sh`):
    ```bash
    ssh -i ~/.ssh/id_rsa_cardcanvas azureuser@<FRONTEND_PUBLIC_IP>
    ```
...
1.  **Transfer Source Code**: Package and transfer the frontend directory:
    ```bash
    COPYFILE_DISABLE=1 tar --exclude=".next" --exclude="node_modules" --exclude=".git" --exclude="._*" -czf frontend-src.tar.gz cardcanvas-frontend
    scp -i ~/.ssh/id_rsa_cardcanvas frontend-src.tar.gz azureuser@<FRONTEND_PUBLIC_IP>:/tmp/
    ```
2.  **Build Code on VM**:
    ```bash
...
2.  **Enable Configuration**:
    ...
    ```
3.  **Acquire Let's Encrypt SSL Certificate (Optional - Requires Custom Domain)**:
    - **If using a custom domain**: Once your custom domain's DNS is pointing to the Frontend VM's Public IP, secure the traffic using HTTPS:
      ```bash
      sudo certbot --nginx -d cardcanvas.com -d www.cardcanvas.com
      # Verify automated renewal is active
      sudo systemctl status certbot.timer
      ```
    - **If testing via VM Public IP (Temporary Setup)**: You can skip this SSL step. Simply access the application at `http://<FRONTEND_PUBLIC_IP>` directly. Note that because you are not using a domain, Nginx will fallback to default HTTP (port 80).
